...
async def confirm(
    ...
    # Add all of our reactions to the message
    await _add_reacts(confirmation_message, sym)
    ...
    # 3. check that the reaction that was added is one we handle for.
    def check(r, u): return (u == target_user) and (
        r.message.id == confirmation_message.id) and (r.emoji in sym)
